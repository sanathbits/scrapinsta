...
async function enableDownloads(page, downloadPath) {
  // Best-effort: allow Chrome downloads into our output folder.
  try {
...
async function clickTurboButtonNearHref(page, href) {
  // On profile grids the extension typically injects a download button near/inside the <a>.
  // We attempt to click any known turbo button class near that href.
  return await page
    .evaluate((h) => {
      const esc = (window.CSS && CSS.escape) ? CSS.escape(h) : h.replace(/"/g, '\\"');
      const a = document.querySelector(`a[href="${esc}"]`);
      if (!a) return { ok: false, reason: "anchor_not_found" };

      // Hover emulation (extension uses :hover CSS to reveal some buttons).
      const hoverTargets = [a, a.parentElement, a.closest("article"), a.closest("div")].filter(Boolean);
      ...
      for (const sel of selectors) {
        const btn =
          a.querySelector(sel) ||
          a.parentElement?.querySelector(sel) ||
          a.closest("article")?.querySelector(sel) ||
          a.closest("div")?.querySelector(sel) ||
          null;
        if (btn) {
          btn.dispatchEvent(new MouseEvent("click", { bubbles: true }));
          return { ok: true, clickedSelector: sel };
        }
      }

      return { ok: false, reason: "download_button_not_found" };
    }, href)
    .catch((e) => ({ ok: false, reason: String(e?.message || e) }));
}

async function clickAnyTurboButtonOnPage(page) {
  // For reel pages, the download button might not be tied to a specific href element.
  return await page
    .evaluate(() => {
      const selectors = [
        ".hover-download-button",
      ...
      return { ok: false, reason: "no_turbo_button_found" };
    })
    .catch((e) => ({ ok: false, reason: String(e?.message || e) }));
}

async function simulateTurboDownloadsForReels(page, username, profileUrl, reelHrefs, outDir) {
  // Assumes we are currently on the user's profile page.
  await enableDownloads(page, outDir);

  const results = [];
  for (const href of reelHrefs) {
    const fullUrl = new URL(href, "https://www.instagram.com").toString();
    const r = { href, fullUrl, ok: false, reason: null };

    try {
      // 1) Try clicking the turbo button near the reel href on the profile page
      await page.evaluate((h) => {
        const esc = (window.CSS && CSS.escape) ? CSS.escape(h) : h.replace(/"/g, '\\"');
        document.querySelector(`a[href="${esc}"]`)?.scrollIntoView({ block: "center" });
      }, href);
      await sleep(300);

      const clickRes1 = await clickTurboButtonNearHref(page, href);
      if (clickRes1.ok) {
        r.ok = true;
        r.reason = null;
        r.clickedSelector = clickRes1.clickedSelector;
        await sleep(1200);
      } else {
        // 2) Fallback: open the reel page and click any turbo button there
        await page.goto(fullUrl, { waitUntil: "domcontentloaded" });
        await sleep(1200);

        // hover the main video area if present (some buttons show on hover)
        await page
          .evaluate(() => {
        ...
        const clickRes2 = await clickAnyTurboButtonOnPage(page);
        r.ok = !!clickRes2.ok;
        r.reason = clickRes2.ok ? null : clickRes2.reason || clickRes1.reason || "unknown";
        r.clickedSelector = clickRes2.clickedSelector;

        // Return to the profile page for the next href
        await page.goto(profileUrl, { waitUntil: "domcontentloaded" });
        await sleep(1200);
      }
    } catch (e) {
      r.ok = false;
      r.reason = String(e?.message || e);
    }
  ...
  return {
    prefix: `/${username}/reel/`,
    attempts: results,
  };
}

async function launchBrowser({ headful, profileDir, args = [] }) {
...
async function main() {
  ...
  // Default to the extension in this repo, but allow override via env.
  const extensionPath = path.resolve(
    process.cwd(),
    process.env.EXTENSION_DIR || "src/extensions/turbo-downloader"
  );
  ...
  // Extensions generally require headful Chrome.
  const wantsExtension = (process.env.LOAD_EXTENSION || "1") === "1";
  const headful = process.env.HEADFUL === "1" || firstRun || wantsExtension;

  const extensionArgs = wantsExtension
    ? [
  ...
  let browser = await launchBrowser({ headful, profileDir, args: extensionArgs });
  ...
  try {
    ...
    // Trigger Turbo Downloader on each reel link (best-effort). Requires extension loaded.
    const turbo = wantsExtension
      ? await simulateTurboDownloadsForReels(page, username, url, filteredLinks, outDir)
      : null;

    const record = { username, url, ...stats, hrefs: filteredLinks, turbo };
